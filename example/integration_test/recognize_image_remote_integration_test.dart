...
void runTests() {
  ...
  test('Image containing bananas test', () async {
    final ByteData data = await rootBundle.load('assets/images/img_banana.jpg');
    ...
    expect(
        result.any((e) =>
            e.foodDataInfo?.foodName.toLowerCase().contains('banana') ?? false),
        isTrue);
  });
  ...
  test('Image not containing any food or food packaging test', () async {
    final ByteData data = await rootBundle.load('assets/images/img_passio.jpg');
    final Uint8List bytes = data.buffer.asUint8List();
  ...
  test(
      ...
      () async {
    final ByteData data = await rootBundle.load('assets/images/img_burger.jpg');
    final Uint8List bytes = data.buffer.asUint8List();
  ...
  test('Image containing eggs without configuring the SDK test', () async {
